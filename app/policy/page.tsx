...
const PolicyPage = () => {
  ...
  const progessRef = useRef<HTMLProgressElement>(null);
  ...
  useEffect(() => {
    window.addEventListener("scroll", (e) => {
      ...
      const current = -mainRef.current.getBoundingClientRect().top;

      progessRef.current!.max = total;
      progessRef.current!.value = current;
    });
  }, []);
  ...
  return (
    <>
      <progress
        className="fixed w-full top-0 h-2 rounded-none"
        ref={progessRef}
        value={0}
      />
      <main ref={mainRef}>
        <section className="flex justify-center bg-primary-foreground">
          <article className="px-5 max-w-[750px] bg-secondary">
            ...
            <p className="mt-2">
              ...
              처리할 수 있도록 하기 위하여 다음과 같은 개인정보 처리방침을
              수립·공개합니다.
            </p>
            ...
            <hgroup>
              ...
              <p className="mt-2">
                [서비스]는 원칙적으로 개인정보 처리 목적이 달성된 경우에는 지체
                없이 해당 개인정보를 파기합니다. 파기의 절차 및 방법은 다음과
                같습니다.
              </p>
              <ul className="mt-1 pl-10 list-disc">
                <li>
